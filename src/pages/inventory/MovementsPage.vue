          ...
          <template v-slot:body-cell-product="props">
            <q-td :props="props">
              <div class="product-info">
                <div class="product-name">{{ props.row.product?.name || $t('common.unknownProduct') }}</div>
        ...
        <q-card-section class="movement-details">
          ...
          <div class="detail-row">
            ...
            <span class="value">
              ...
              <span class="sku">({{ selectedMovement.product?.sku || 'No SKU' }})</span>
            </span>
          </div>

          <div class="detail-row">
            <span class="label">{{ $t('inventory.location') }}:</span>
            <span class="value">{{ selectedMovement.location?.name || 'Unknown Location' }}</span>
          </div>
          ...
          <div class="detail-row">
            ...
            >
              {{ selectedMovement.quantity_change > 0 ? '+' : '' }}{{ selectedMovement.quantity_change }}
            </span>
          </div>
          ...
          <div v-if="selectedMovement.reason_code" class="detail-row">
            ...
            <span class="value">{{ formatReasonCode(selectedMovement.reason_code) }}</span>
          </div>
          ...
          <div class="detail-row">
            ...
            <span class="value">{{ formatDateTime(selectedMovement.created_at) }}</span>
          </div>
        </q-card-section>
      </q-card>
    </q-dialog>
...
import type { MovementWithRelations, MovementType, ReasonCode } from 'src/types/inventory'
...
const filteredMovements = computed(() => {
  ...
  if (selectedMovementType.value) {
    movements = movements.filter(m => m.movement_type === selectedMovementType.value)
  }

  // Filter by location
  if (selectedLocationId.value) {
    movements = movements.filter(m => m.location_id === selectedLocationId.value)
  }
