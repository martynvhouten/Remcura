    ...
    <div class="page-header">
      <div class="header-content">
        <div class="breadcrumb-nav">
          ...
          <span class="breadcrumb-current">{{ orderList?.name || 'Laden...' }}</span>
        </div>
        
        <div class="header-main">
          <div class="title-section">
            <div class="list-icon-large">
              <q-icon :name="getListIcon()" :color="getIconColor()" size="32px" />
            </div>
            <div class="title-content">
              <h1 class="page-title">{{ orderList?.name || 'Bestellijst' }}</h1>
              <p class="page-subtitle">{{ orderList?.description || 'Details en producten beheren' }}</p>
              <div class="title-meta">
                <q-badge :color="statusColor" :label="statusLabel" class="status-badge" />
                <span class="meta-separator">•</span>
                <span class="meta-text">{{ orderList?.total_items || 0 }} producten</span>
                <span class="meta-separator">•</span>
                <span class="meta-text">€{{ formatCurrency((orderList as any)?.total_value || 0) }}</span>
              </div>
            </div>
          </div>
            ...
            >
              <q-menu>
                <q-list dense>
                  ...
                  <q-separator />
                  <q-item clickable @click="deleteList" v-close-popup class="text-negative">
                    <q-item-section avatar>
                      <q-icon name="delete" />
                    </q-item-section>
...
const filterProducts = (val: string, update: (callback: () => void) => void) => {
  update(() => {
    if (val === '') {
      ...
      filteredProducts.value = productsStore.products.filter(
        product => 
...
const removeItem = async (item: any) => {
  $q.dialog({
    ...
    message: `Weet je zeker dat je "${item.product_name || item.product?.name}" wilt verwijderen?`,
    cancel: true,
    persistent: true,
  }).onOk(async () => {
    try {
