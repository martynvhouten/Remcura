...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const lowStockItems = computed(() => {
    return stockLevels.value.filter(stock => 
  ...
  const fetchStockLevels = async (practiceId: string) => {
    ...
    try {
      const { data, error } = await supabase
        .rpc('get_stock_overview', { p_practice_id: practiceId })
  ...
  const fetchOrderSuggestions = async (practiceId: string) => {
    try {
      const { data, error } = await supabase
        .rpc('generate_order_suggestions', { p_practice_id: practiceId })
  ...
  const generateStockAlerts = () => {
    ...
    stockLevels.value.forEach(stock => {
      ...
      // Low stock alerts
      else if (stock.current_quantity <= stock.minimum_stock && stock.minimum_stock > 0) {
        const urgency = stock.current_quantity <= stock.minimum_stock * 0.5 ? 'high' : 'medium'
        alerts.push({
          type: 'low_stock',
          urgency,
          product_id: stock.product_id,
      ...
      // Overstock alerts
      else if (stock.maximum_stock > 0 && stock.current_quantity > stock.maximum_stock) {
        alerts.push({
          type: 'overstock',
          urgency: 'low',
          product_id: stock.product_id,
  ...
  const getInventoryKPIs = async (practiceId: string): Promise<InventoryKPI> => {
    try {
