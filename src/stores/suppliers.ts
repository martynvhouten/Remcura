...
export const useSuppliersStore = defineStore('suppliers', () => {
  ...
  const getSupplierById = computed(() => (id: string) =>
    suppliers.value.find(supplier => supplier.id === id)
  )

  const getSupplierProductsBySupplier = computed(() => (supplierId: string) =>
    supplierProducts.value.filter(sp => sp.supplier_id === supplierId)
  )

  const getSupplierProductsForProduct = computed(() => (productId: string) =>
  ...
  const createSupplierProduct = async (supplierProductData: Omit<SupplierProduct, 'id' | 'created_at' | 'updated_at' | 'last_price_update'>) => {
    try {
      const { data, error } = await supabase
        .from('supplier_products')
        .insert([{
          ...
          minimum_order_quantity: supplierProductData.minimum_order_quantity || 1,
          ...
          is_backorder_allowed: supplierProductData.is_backorder_allowed ?? false,
  ...
  const updateSupplierProduct = async (id: string, updates: Partial<SupplierProduct>) => {
    try {
      const { data, error } = await supabase
        .from('supplier_products')
        .update({
          ...updates,
          last_price_update: updates.unit_price !== undefined ? new Date().toISOString() : undefined
        })
        .eq('id', id)
        .select()
        .single()
  ...
  const deleteSupplierProduct = async (id: string) => {
    try {
      ...
      supplierProducts.value = supplierProducts.value.filter(sp => sp.id !== id)
    } catch (error) {
  ...
  const findBestSupplierForProduct = (productId: string) => {
    const productSuppliers = supplierProducts.value.filter(sp => 
      sp.product_id === productId && sp.is_available
    )
