...
export const useCountingStore = defineStore('counting', () => {
  ...
  const countingStats = computed((): CountingStats => {
    ...
    return {
      ...
      discrepancies,
      progress_percentage: totalProducts > 0 ? (countedProducts / totalProducts) * 100 : 0
    }
  })
  ...
  const entriesPendingVerification = computed(() =>
    countingEntries.value.filter(entry => entry.status === 'pending' || entry.status === 'discrepancy')
  )

  const nextProductToCount = computed(() => {
    if (!availableProducts.value.length) return null
    
    const countedProductIds = countingEntries.value.map(entry => entry.product_id)
  ...
  const fetchProductsForSession = async (sessionId: string) => {
    try {
      const session = currentSession.value || sessions.value.find(s => s.id === sessionId)
  ...
  const countProduct = async (
    ...
    try {
      ...
      const newDiscrepanciesCount = Math.abs(variance) > 0 
  ...
  const updateCountingEntry = async (entryId: string, updates: Partial<CountingEntry>) => {
    try {
      ...
      const index = countingEntries.value.findIndex(entry => entry.id === entryId)
      if (index >= 0) {
  ...
  const updateSession = async (sessionId: string, updates: Partial<CountingSession>) => {
    try {
      const { data, error } = await supabase
        .from('counting_sessions')
        .update(updates)
