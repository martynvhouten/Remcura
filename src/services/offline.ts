...
export class OfflineService {
  ...
  async downloadLatestData(): Promise<void> {
    ...
    try {
      ...
      this.offlineData.bestellijst_items = items?.map(item => ({
        ...item,
  ...
  async syncToServer(): Promise<void> {
    if (!this.isOnline.value || this.syncInProgress.value || this.offlineActions.value.length === 0) {
      return
    }
    ...
    try {
      console.log(`Syncing ${this.offlineActions.value.length} offline actions...`)
      ...
      failedActions.forEach(failedAction => {
        const index = this.offlineActions.value.findIndex(a => a.id === failedAction.id)
        if (index !== -1) {
      ...
      console.log(`Sync completed. Synced: ${syncedActions.length}, Failed: ${failedActions.length}`)
   ...
   */
  updateBestellijstItemOffline(itemId: string, updates: Partial<BestellijstItem>): void {
    // Update local data
    const index = this.offlineData.bestellijst_items.findIndex(item => item.id === itemId)
    if (index !== -1) {
  ...
  private saveOfflineData(): void {
    try {
      localStorage.setItem('medstock_offline_data', JSON.stringify({
        ...this.offlineData,
  ...
  private saveOfflineActions(): void {
    try {
      localStorage.setItem('medstock_offline_actions', JSON.stringify(
