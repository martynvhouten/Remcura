...
export class NotificationService {
   ...
   */
  async showBrowserNotification(notification: NotificationMessage): Promise<void> {
  ...
  async updateNotificationSetting(
    ...
    this.settings[key] = { ...settingData, id: crypto.randomUUID(), created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
  }

  /**
   * Check if notification is enabled
   */
  isNotificationEnabled(notificationType: NotificationType, channel: NotificationChannel): boolean {
    const key = `${notificationType}_${channel}`
   ...
   */
  async sendStockAlert(productName: string, currentStock: number, minimumStock: number): Promise<void> {
   ...
   */
  async sendOrderUpdate(orderNumber: string, status: string, message?: string): Promise<void> {
    const notification: NotificationMessage = {
   ...
   */
  async sendReminder(title: string, message: string, data?: any): Promise<void> {
    const notification: NotificationMessage = {
      title,
      body: message,
      type: 'reminder',
