<template>
  <div class="modern-order-card">
    ...
    <div class="card-header">
      <div class="header-left">
        ...
        <div class="list-info">
          ...
          <p class="list-subtitle">{{ orderList.description || 'Geen beschrijving' }}</p>
          </div>
        </div>
    ...
    <!-- Tags Row -->
    <div v-if="(orderList as any).tags && (orderList as any).tags.length > 0" class="tags-section">
      <q-chip
    ...
    <q-menu v-model="showActionsMenu" anchor="top right" self="top right">
      <q-list dense class="actions-menu">
        ...
        <q-item clickable @click="editSettings" class="text-primary" v-close-popup>
                  <q-item-section avatar>
    ...
    <div class="card-content">
      ...
      <div v-if="reorderAdvice && reorderAdvice.items_by_urgency.critical.length > 0" class="critical-alert">
        <div class="alert-icon">
          <q-icon name="warning" color="negative" size="20px" />
        </div>
        <div class="alert-content">
          <span class="alert-text">{{ reorderAdvice.items_by_urgency.critical.length }} kritieke items</span>
              <q-btn 
                flat 
            dense
                label="Bestel nu" 
    ...
    <div class="order-recommendations q-pt-none" v-if="showRecommendations">
      ...
      <!-- Supplier breakdown -->
      <div v-if="reorderAdvice && reorderAdvice.suppliers_involved.length > 0" class="suppliers-breakdown q-mb-md">
        <div class="text-caption text-grey-6 q-mb-xs">Leveranciers betrokken:</div>
          ...
          >
            ...
            <q-item-section>
              ...
              <q-item-label caption>
                Voorraad: {{ item.current_stock }} / Min: {{ item.minimum_stock }}
              </q-item-label>
            </q-item-section>
            
            <q-item-section side>
              <div class="text-right">
                <div class="text-weight-bold text-primary">{{ item.calculated_order_quantity }}</div>
      ...
      <div v-if="reorderAdvice" class="cost-estimation q-mt-md">
        <div class="row items-center">
          <div class="col">
            ...
            <div class="text-h6 text-primary">€{{ reorderAdvice.total_estimated_cost.toFixed(2) }}</div>
          </div>
          <div class="col-auto">
            <div class="text-caption text-grey-6">Items:</div>
            <div class="text-subtitle1">{{ reorderAdvice.total_items_to_order }}</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="(orderList as any).auto_reorder_enabled" class="auto-reorder-progress">
      <q-linear-progress
...
import type { OrderAdvice, ReorderSuggestion } from '@/stores/orderLists/orderLists-minmax';
...
const outOfStockCount = computed(() => {
  return orderListsStore.orderSuggestions?.filter(item => 
...
const autoReorderProgress = computed(() => {
  ...
  const lastCheck = (props.orderList as any).last_auto_check ? 
    new Date((props.orderList as any).last_auto_check) : new Date();
...
const nextAutoCheck = computed(() => {
  ...
  nextCheck.setDate(nextCheck.getDate() + ((props.orderList as any).reorder_frequency_days || 14));
...
const orderAllItems = async () => {
  creatingOrder.value = true;
  try {
    const allItems = orderListsStore.orderSuggestions?.filter(item => 
      item.order_list_id === props.orderList.id
    ) || [];
...
const duplicateList = async () => {
  try {
    const newList = await orderListsStore.duplicateOrderList(props.orderList.id);
...
.stat-success .stat-value { color: #388e3c; }
.stat-info .stat-value { color: #1976d2; }
