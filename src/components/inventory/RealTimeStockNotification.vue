...
interface StockNotification {
  id: string;
  type: 'stock_update' | 'stock_low' | 'stock_out' | 'transfer' | 'adjustment';
  title: string;
  message: string;
  productId?: string;
  productName?: string;
...
const addNotification = (notification: Omit<StockNotification, 'id' | 'timestamp'>) => {
...
onMounted(() => {
  ...
  const interval = setInterval(() => {
    if (Math.random() > 0.7) { // 30% chance every 10 seconds for demo
      addNotification({
        type: 'stock_update',
