    ...
    <BaseCard class="entry-card">
      <q-card-section class="entry-header">
        <div class="flex justify-between items-center">
          <div class="product-info">
            ...
            <div class="text-caption">{{ product.sku }} • {{ product.category }}</div>
          </div>
          <q-chip 
            :color="statusColor" 
            :icon="statusIcon" 
      ...
      <q-card-section class="entry-content">
        ...
        <div class="current-stock-section q-mb-md">
          <div class="text-subtitle2 q-mb-sm">{{ $t('inventory.currentStock') }}</div>
        ...
        <div v-if="viewMode === 'full' || hasBatches" class="batch-section q-mb-md">
          <div class="text-subtitle2 q-mb-sm">{{ $t('batch.batchInformation') }}</div>
          
          <!-- Existing Batches -->
          <div v-if="existingBatches.length > 0" class="existing-batches q-mb-md">
            <div class="text-caption q-mb-xs">{{ $t('batch.existingBatches') }}:</div>
                  ...
                  <div class="flex justify-between items-center">
                    <div>
                      ...
                      <div class="text-caption">
                        {{ $t('batch.expires') }}: {{ formatDate(batch.expiry_date) }}
                      </div>
                    </div>
                    <div class="text-right">
                      <div class="text-weight-medium">{{ batch.current_quantity }}</div>
          ...
          <div v-if="viewMode === 'full' || addingNewBatch" class="new-batch-input">
            <div class="text-caption q-mb-xs">{{ $t('batch.addNewBatch') }}:</div>
            <BatchInput
        ...
        <div class="counted-quantity-section">
          <div class="text-subtitle2 q-mb-sm">{{ $t('counting.countedQuantity') }}</div>
          <q-input
        ...
        <div v-if="hasDiscrepancy" class="discrepancy-section q-mt-md">
          ...
          >
            ...
            <div class="text-body2">
              {{ $t('counting.difference') }}: {{ discrepancyAmount }} {{ product.unit }}
            </div>
          </q-banner>
        </div>
...
import type { Product, ProductBatchWithDetails, CountingEntry } from 'src/types/inventory';
...
const existingBatches = computed(() => {
  return batchStore.batchesByProduct(props.product.id).filter(
    batch => batch.location_id === props.locationId
  );
});
...
const currentStock = computed(() => {
  return existingBatches.value.reduce((total, batch) => total + batch.current_quantity, 0);
});

const hasDiscrepancy = computed(() => {
  return countedQuantity.value !== null && countedQuantity.value !== currentStock.value;
});
...
const discrepancyClass = computed(() => {
  ...
  return discrepancyAmount.value > 0 ? 'bg-orange-1 text-orange-8' : 'bg-red-1 text-red-8';
});
...
const isValid = computed(() => {
  const hasValidCount = countedQuantity.value !== null && countedQuantity.value >= 0;
...
if (props.existingEntry) {
  ...
  if (props.existingEntry.batch_id) {
    selectedBatch.value = existingBatches.value.find(b => b.id === props.existingEntry.batch_id) || null;
  }
}
</script>
