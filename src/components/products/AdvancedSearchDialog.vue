    ...
    <q-card class="advanced-search-card" style="min-width: 700px; max-width: 900px;">
      ...
      <q-card-section class="q-pt-none">
        <div class="advanced-search-content">
          ...
          <!-- Category & Supplier -->
          <div class="search-section">
            <h6 class="section-title">{{ $t('productsPage.advancedSearch.categorySupplier') }}</h6>
            <div class="search-grid">
              <q-select
          ...
          <!-- Price Range -->
          <div class="search-section">
            <h6 class="section-title">{{ $t('productsPage.advancedSearch.priceRange') }}</h6>
          ...
          <!-- Special Options -->
          <div class="search-section">
            <h6 class="section-title">{{ $t('productsPage.advancedSearch.specialOptions') }}</h6>
          ...
          <!-- Sorting -->
          <div class="search-section">
            <h6 class="section-title">{{ $t('productsPage.advancedSearch.sorting') }}</h6>
            ...
            <div class="preview-content">
              <div class="preview-stats">
                <q-chip
                  ...
                  size="md"
                >
                  {{ $t('productsPage.advancedSearch.resultsFound', { count: previewResults.count }) }}
                </q-chip>
                ...
                <div v-if="previewResults.count > 0" class="preview-metrics">
                  <q-chip
                    ...
                    size="sm"
                  >
                    {{ $t('productsPage.advancedSearch.avgPrice') }}: {{ formatPrice(previewResults.averagePrice) }}
                  </q-chip>
                  ...
                  <q-chip
                    ...
                    size="sm"
                  >
                    {{ $t('productsPage.advancedSearch.inStock') }}: {{ previewResults.inStockCount }}
                  </q-chip>
                  ...
                  <q-chip
                    ...
                    size="sm"
                  >
                    {{ $t('productsPage.advancedSearch.withGtin') }}: {{ previewResults.gtinCount }}
                  </q-chip>
                </div>
              </div>

              <q-slide-transition>
                <div v-show="previewExpanded && previewResults.samples.length > 0" class="preview-samples">
                    ...
                    >
                      <div class="sample-info">
                        ...
                        <div class="sample-details">
                          <span v-if="product.sku" class="sample-sku">{{ product.sku }}</span>
                          <span v-if="product.category" class="sample-category">{{ product.category }}</span>
...
<script setup lang="ts">
  ...
  interface Emits {
    ...
    (e: 'search', criteria: ProductFilter & { priceMin?: number; priceMax?: number; has_gtin?: boolean; batch_tracked?: boolean; in_stock_only?: boolean }): void;
    (e: 'open-scanner'): void;
  ...
  const sortOrderOptions = computed(() => [
    { label: t('productsPage.sortOrder.asc'), value: 'asc', icon: 'arrow_upward' },
    { label: t('productsPage.sortOrder.desc'), value: 'desc', icon: 'arrow_downward' },
  ]);
  ...
  const showPreview = computed(() => {
    return hasValidCriteria.value && (previewResults.value.count > 0 || previewResults.value.count === 0);
  });
  ...
  watch(searchCriteria, (newCriteria) => {
