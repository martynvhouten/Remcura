      ...
      <div class="version-section">
        ...
        <div class="version-details">
          <div class="version-item">
            <span class="version-label">{{ $t('platform.system.appVersion') }}</span>
          ...
          <div class="version-item">
            <span class="version-label">{{ $t('platform.system.buildNumber') }}</span>
            <span class="version-value">#{{ data.build_number }}</span>
          </div>
          
          <div class="version-item">
            <span class="version-label">{{ $t('platform.system.environment') }}</span>
            <q-chip 
          ...
          <div class="version-item">
            <span class="version-label">{{ $t('platform.system.lastDeployment') }}</span>
            <span class="version-value">{{ formatDateTime(data.last_deployment) }}</span>
          </div>
        </div>
      ...
      <div class="health-section">
        ...
        <div class="health-metrics">
          <div class="health-item">
            <div class="health-label">{{ $t('platform.system.overallStatus') }}</div>
            <q-chip 
          ...
          <div class="health-item">
            <div class="health-label">{{ $t('platform.system.errors24h') }}</div>
            <div class="health-value text-h6">{{ data.error_count_24h || 0 }}</div>
          </div>
          
          <div class="health-item">
            <div class="health-label">{{ $t('platform.system.activePractices') }}</div>
            <div class="health-value text-h6">{{ data.active_practices_1h || 0 }}</div>
          </div>
          
          <div class="health-item">
            <div class="health-label">{{ $t('platform.system.databaseStatus') }}</div>
            <q-chip 
              :color="data.database_status === 'connected' ? 'positive' : 'negative'" 
              text-color="white"
              :icon="data.database_status === 'connected' ? 'check_circle' : 'error'"
              class="health-value"
            >
        ...
        <div class="health-footer">
          <div class="text-caption text-grey-6">
            {{ $t('platform.system.lastCheck') }}: {{ formatDateTime(data.last_check) }}
          </div>
        </div>
      </div>
    </template>
    ...
    <template v-else-if="isDatabaseStatus">
      <div class="database-section">
        <div class="database-header">
          ...
          <span class="text-h6">{{ $t('platform.system.databaseStatus') }}</span>
        </div>
        ...
        <div class="database-metrics">
          ...
          <template v-if="data.response_time_ms">
            <div class="database-item">
              ...
              <span>{{ $t('platform.system.responseTime') }}: {{ data.response_time_ms }}ms</span>
            </div>
          </template>
          ...
          <template v-if="data.total_tables">
            <div class="database-item">
              ...
              <span>{{ $t('platform.system.totalTables') }}: {{ data.total_tables }}</span>
            </div>
          </template>
          ...
          <template v-if="data.largest_table">
            <div class="database-item">
              ...
              <span>{{ $t('platform.system.largestTable') }}: {{ data.largest_table }}</span>
            </div>
          </template>
        </div>
...
const isVersionInfo = computed(() => {
  return props.data.app_version !== undefined || props.data.build_number !== undefined;
});

const isSystemHealth = computed(() => {
  return props.data.overall_status !== undefined || props.data.error_count_24h !== undefined;
});

const isDatabaseStatus = computed(() => {
  return props.data.status !== undefined && props.data.response_time_ms !== undefined;
});
...
const filteredData = computed(() => {
  ...
  const excludeKeys = ['dependencies', 'last_check', 'overall_status', 'error_count_24h'];
