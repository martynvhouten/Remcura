  ...
  <div class="batch-input-container">
    <div class="batch-input-wrapper">
        ...
        <template #option="{ option, selected, focused }">
          <q-item v-bind="{ selected, focused }" clickable>
            <q-item-section>
              ...
              <q-item-label caption>{{ option.sku }} • {{ option.category }}</q-item-label>
            </q-item-section>
          </q-item>
        </template>
      </BaseSelect>
      ...
      <div v-if="existingBatches.length > 0" class="existing-batches">
        <div class="text-subtitle2 q-mb-md">{{ $t('batch.existingBatches') }}</div>
        <q-card v-for="batch in existingBatches" :key="batch.id" flat bordered class="batch-card">
      ...
      <div v-if="viewMode === 'lite'" class="batch-summary">
        <q-card flat bordered>
          <q-card-section class="q-pa-md">
            <div class="text-subtitle2 q-mb-sm">{{ $t('batch.batchSummary') }}</div>
...
const isValid = computed(() => {
  ...
  return !!(selectedProduct.value && batchNumber.value && expiryDate.value && quantity.value);
});

// Validation rules
const required = (val: any) => (!!val && val !== '') || t('validation.required');
...
const searchExistingBatches = async () => {
  ...
  try {
    ...
    const batches = batchStore.batchesByProduct(selectedProduct.value.id).filter(
      batch => batch.batch_number.toLowerCase().includes(batchNumber.value.toLowerCase())
    );
...
watch(() => props.modelValue, (newValue) => {
  if (newValue) {
    if (newValue.productId) {
      const product = productStore.activeProducts.find(p => p.id === newValue.productId);
