        ...
        <template v-for="section in navigationLinks" :key="section.id">
          ...
          <template v-for="item in section.items" :key="item.title">
            <q-item
              ...
              :aria-label="item.title + (item.badge ? ' (' + item.badge + ' items)' : '')"
            ...
            >
              ...
              <q-item-section>
                <q-item-label class="nav-item-label">{{ item.title }}</q-item-label>
              </q-item-section>
              ...
              <q-item-section side v-if="item.submenu" class="submenu-chevron">
                <q-icon
                  :name="isSubmenuExpanded(item.routeName) ? 'expand_less' : 'expand_more'"
                  size="20px"
                  class="chevron-icon"
                />
              </q-item-section>
            ...
            <q-slide-transition v-if="item.submenu">
              <div v-show="isSubmenuExpanded(item.routeName)">
                ...
                >
                  ...
                  <q-item-section>
                    <q-item-label class="nav-item-label">{{ subItem.title }}</q-item-label>
                  </q-item-section>
                </q-item>
              </div>
            </q-slide-transition>
          ...
          <q-separator 
            v-if="section.id !== navigationLinks[navigationLinks.length - 1]?.id" 
...
const checkAndExpandCurrentSubmenu = () => {
  ...
  for (const section of sections) {
    for (const item of section.items) {
      if (item.submenu && item.submenu.length > 0) {
        const hasActiveChild = item.submenu.some((subItem: NavigationItem) => subItem.routeName === currentRoute);
...
const isParentActive = (item: NavigationItem) => {
  ...
  const currentRoute = router.currentRoute.value.name as string;
  return item.submenu.some((subItem: NavigationItem) => subItem.routeName === currentRoute);
};
