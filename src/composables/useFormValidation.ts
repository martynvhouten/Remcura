  ...
  const rules = {
    ...
    minLength:
      ...
      (value: string): boolean | string => {
        return (
          (value && value.length >= min) || t('validation.minLength', { min })
        );
      },
    ...
    maxLength:
      ...
      (value: string): boolean | string => {
        return (
          !value || value.length <= max || t('validation.maxLength', { max })
        );
      },
    ...
    integer: (value: any): boolean | string => {
      return (
        !value || Number.isInteger(Number(value)) || t('validation.integer')
      );
    },
  };
