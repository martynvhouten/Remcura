...
import { validateGTIN, parseBoolean, parseDate, parseNetContent, mapGS1ToProduct } from './importGs1Excel';
...
testContents.forEach(content => {
  ...
  console.log(`  ${content.padEnd(10)} → value: ${parsed.value || 'N/A'}, uom: ${parsed.uom || 'N/A'}`);
});
...
if (mappedProduct) {
  ...
  console.log(`  Net Content: ${mappedProduct.net_content_value}${mappedProduct.net_content_uom}`);
  console.log(`  Unit Indicators: Base=${mappedProduct.base_unit_indicator}, Order=${mappedProduct.orderable_unit_indicator}, Despatch=${mappedProduct.despatch_unit_indicator}`);
  console.log(`  Dates: ${mappedProduct.effective_from_date} to ${mappedProduct.effective_to_date}`);
} else {
...
console.log(mappedInvalidProduct ? '❌ Should have failed but didn\'t' : '✅ Correctly rejected invalid row');

console.log('\n🎉 All tests completed!');
console.log('\n💡 Usage examples:');
console.log('  npm run gs1:import                                    # Import default file');
console.log('  npx ts-node scripts/importGs1Excel.ts myfile.xlsx     # Import custom file');
console.log('  npx ts-node scripts/importGs1Excel.ts file.xlsx "UK"  # Import specific sheet'); 